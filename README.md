...
**Warimoney**は、グループで発生した支出を記録し、効率的に清算できるWebアプリケーションです。複数人での旅行や食事、イベント開催時に、誰がいくら払ったか、誰がいくら返すべきかを自動計算し、清算します。ユーザー登録・認証機能を備え、プロジェクト単位で支出管理を行う設計になっています。
...
4. **ロジック実装**：清算計算エンジンの設計（SettlementService）
5. **JPA/Hibernateの活用**：ORM技術の実践的な使用

反省点：
1. 設計が甘く、アドリブな部分が多かった。
2. 一人で作成したため、コードレビューができない。
3. デザイン等調整の区切りがつけられない
